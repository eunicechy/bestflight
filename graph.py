...
class Graph:
    ...
    def short_path(self, dest, sour="Malaysia", routes=None):
        if (routes == None):
            sour = self._get_airport(sour)
            dest = self._get_airport(dest)
            routes = [[]]
    ...
    def compute_path(self, dest, routes):
        ...
        if (True):
            ...
            for j in range(len(self.adjList[n])):
                path = routes[0].copy()
                distance = round(self.adjList[n][j][1], 2)
    ...
    def _political_score(self, airport):
        ...
        if (city not in self.cityps):
            ps = posen.Ps(city)
            ps.calculate_political_score()
            score = ps.get_political_score()
            self.cityps[city] = score
            ps.print_graph1()
        return self.cityps[city]

    def selection(self, des):
        self.short_path(des)
        # print(self.pathList)
        # for p in self.pathList:
        #     print("Distance: {}".format(round(p[0], 2)))
        #     print("Path: ", end=" ")
        #     total = 0
        #     result = ""
        #     for i in range(1, len(p)):
        #         if(1<i<len(p)-1):
        #             score = self._political_score(p[i])
        #             total += score
        #             result += p[i]+": "+str(score)
        #         print(p[i], end=" ")
        #     print(total)
        #     print(result)
    ...
    def print_map(self, path):
        ...
        for i in range(1, len(path)):
            points.append(
                (self.geolocation(path[i]).latitude, self.geolocation(path[i]).longitude))
            folium.Marker(points[i - 1], popup=path[i]).add_to(m)
        folium.PolyLine(points, color="blue", weight=5, popup="").add_to(m)

        m.save('map.html')
        # Open the map in Chrome browser in Windows
        chrome_path = 'C:/Program Files (x86)/Google/Chrome/Application/chrome.exe %s'
        webbrowser.get(chrome_path).open("file://D://UM FSKTM//Sem4 19//WIA2005//Assignment//map.html", 2, True)